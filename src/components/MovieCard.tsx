...
export const MovieCard = memo(
  ({ movie, index = 0, size = "md" }: MovieCardProps) => {
    ...
    // Runflix-inspired quality detection parameter [2]
    const quality = movie.quality || movie.resolution;
    ...
    if (isSeries && prog) {
      ...
      const duration = localStorage.getItem(
        `rf_progress_${episodeTitle}_duration`,
      );
          ...
          <div className="p-3 bg-gradient-to-t from-black via-black/95 to-black/80 border-t border-white/[0.04] relative z-[2]">
            ...
            <div className="flex items-center justify-between mt-1">
              <div className="flex items-center gap-1.5">
                ...
                {quality && (
                  <span className="text-[8px] font-black text-white/50 px-1 py-[1px] rounded bg-white/[0.05] border border-white/5 uppercase shrink-0">
                    {quality}
                  </span>
                )}
              </div>
