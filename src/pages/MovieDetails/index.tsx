...
import { usePrefetchSources } from "./hooks/usePrefetchSources";
...
export default function MovieDetails() {
  ...
  const [downloadMode,      setDownloadMode]      = useState<"single" | "batch" | "all">("single");
  ...
  // Current season max episodes
  const currentSeason = seasons.find((s) => s.se === selectedSeason);
  const maxEp         = currentSeason?.maxEp || 0;

  // Standard sources query (fires when modal opens)
  ...
  // ── Background prefetch — runs as soon as page loads ─────────────────────────
  // Sources and batch data are fetched silently so modal opens instantly.
  const { prefetchedSources, prefetchedBatch, sourcesReady, batchReady } =
    usePrefetchSources(id, isTvSeries, selectedSeason, selectedEpisode, maxEp);
  ...
  const seoSchema = useMemo(() => {
    ...
    return {
      ...
      image: img,
      ...(rating
        ? { aggregateRating: { "@type": "AggregateRating", ratingValue: rating, bestRating: "10", worstRating: "1", ratingCount: 1500 } }
        : {}),
  ...
  // Auto-select season/episode from progress
  useEffect(() => {
  ...
  const handleModeChange = (mode: "single" | "batch" | "all") => {
    setDownloadMode(mode);
    if (mode === "single") {
      resetBatch();
    } else {
      // If prefetch already finished → populate batch instantly, no spinner
      if (batchReady && prefetchedBatch.length > 0) {
        fetchBatch(maxEp, prefetchedBatch);
      } else {
        // Prefetch still running → trigger fresh chunked fetch
        fetchBatch(maxEp);
      }
    }
  };
      ...
      <div className="min-h-screen flex items-center justify-center p-8">
        ...
        >
          ...
          <p className="text-sm text-[var(--rf-text-muted)] mb-6">
            {error ? String((error as any)?.message) : "This content could not be loaded."}
          </p>
          <div className="flex items-center justify-center gap-3">
            <button onClick={() => navigate("/")} className="btn-glass px-6 py-3 text-sm">
  ...
  const hasMovieProgress  =
    !isTvSeries && prog && prog.lastTime && prog.duration &&
    prog.lastTime > 5 && prog.lastTime < prog.duration - 15;
    ...
    <div className="min-h-screen relative -mt-[72px]">
      ...
      <div className="absolute inset-0 h-[70vh] overflow-hidden">
        ...
        <div className="absolute inset-0"
          style={{ background: "radial-gradient(ellipse at center, transparent 30%, var(--rf-black) 100%)" }} />
      </div>
                ...
                {Array.from({ length: maxEp }, (_, i) => i + 1).map((ep) => {
                  ...
                  if (isCompleted) {
                    ...
                    if (savedTime && duration) {
                      const t = parseFloat(savedTime), d = parseFloat(duration);
                  ...
                  return (
                    <div
                      key={ep}
                      onClick={() => {
                        trackPlay(subject.subjectId || id, `${subject.title} S${selectedSeason}E${ep}`);
                    ...
                    >
                      <div className="relative aspect-video w-full bg-white/5 overflow-hidden">
                        <img src={backdropUrl || coverUrl} alt={`Episode ${ep}`}
                        ...
                        {realEpProgress > 0 && (
                          <div className="absolute bottom-0 left-0 w-full h-1 bg-white/20 z-[3]">
                            <div className="h-full bg-[var(--rf-red)] shadow-[0_0_8px_var(--rf-red)] transition-all duration-300"
                      ...
                      <div className="p-3 flex-1 flex flex-col justify-between">
                        <div>
                          <h4 className="text-xs font-bold text-white group-hover:text-[var(--rf-red)] transition-colors line-clamp-1">Episode {ep}</h4>
                          <p className="text-[10px] text-[var(--rf-text-muted)] mt-0.5 uppercase tracking-wider">Season {selectedSeason}</p>
                        </div>
                        {isCompleted && (
                          <div className="mt-2 text-[9px] font-bold text-green-400 flex items-center gap-1">✓ Watched</div>
                        )}
                      </div>
                    </div>
                  );
      ...
      {/* Download Modal — uses prefetched data for instant load */}
      <DownloadModal
        ...
        // Use prefetched sources if ready, otherwise fall back to query data
        sources={sourcesReady && prefetchedSources ? prefetchedSources.sources : sources}
        subtitles={sourcesReady && prefetchedSources ? prefetchedSources.subtitles : subtitles}
        sourcesLoading={sourcesReady ? false : sourcesLoading}
        // Use prefetched batch if ready
        batchResults={batchReady && prefetchedBatch.length > 0 ? prefetchedBatch : batchResults}
        batchLoading={batchReady ? false : batchLoading}
        onSeasonChange={(s) => { setSelectedSeason(s); setSelectedEpisode(1); resetBatch(); }}
        onEpisodeChange={setSelectedEpisode}
        onModeChange={handleModeChange}
        onFetchBatch={() => {
          // Pass prefetched batch if available — instant, no spinner
          fetchBatch(maxEp, batchReady ? prefetchedBatch : undefined);
        }}
