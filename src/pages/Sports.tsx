...
export default function Sports() {
  ...
  const [standingsSubTab, setStandingsSubTab] = useState<
    "nba" | "nfl" | "soccer"
  >("soccer");
  ...
  const normalizeScoreItem = (
    ...
    const homeTeamStr = homeObj
      ? homeObj.name || homeObj.shortName
      : item.homeTeam || item.home || "Home Team";
    const awayTeamStr = awayObj
      ? awayObj.name || awayObj.shortName
      : item.awayTeam || item.away || "Away Team";
    ...
    return {
      ...
      homeLogo:
        homeObj && homeObj.logo
          ? homeObj.logo
          : item.homeLogo ||
            `https://ui-avatars.com/api/?name=${encodeURIComponent(homeTeamStr)}&background=1e293b&color=38bdf8&bold=true`,
      awayTeam: awayTeamStr,
      awayLogo:
        awayObj && awayObj.logo
          ? awayObj.logo
          : item.awayLogo ||
            `https://ui-avatars.com/api/?name=${encodeURIComponent(awayTeamStr)}&background=1e293b&color=f43f5e&bold=true`,
      homeScore,
      awayScore,
    ...
    try {
      ...
      try {
        const liveRes = await fetch(
          "https://apis.davidcyril.name.ng/sports/live",
        );
        if (liveRes.ok) {
          ...
          if (
            ...
            setLiveScores(
              liveData.map((item: any, idx: number) =>
                normalizeScoreItem(item, idx, "LIVE"),
              ),
            );
          }
        }
      ...
      try {
        const soccerRes = await fetch(
          `https://apis.davidcyril.name.ng/sports/soccer/scores?league=${soccerLeague}`,
        );
        if (soccerRes.ok) {
          const soccerData = await soccerRes.json();
          const games = Array.isArray(soccerData)
            ? soccerData
            : soccerData.games || [];
            ...
            pastResults.push(
              ...games
                ...
                .map((item: any, idx: number) =>
                  normalizeScoreItem(item, idx, "FINISHED", "Soccer"),
                ),
            );
          }
        }
      ...
      try {
        const nbaRes = await fetch(
          "https://apis.davidcyril.name.ng/sports/nba/scores",
        );
        if (nbaRes.ok) {
          ...
          if (Array.isArray(games) && games.length > 0) {
            pastResults.push(
              ...games
                .slice(0, 10)
                .map((item: any, idx: number) =>
                  normalizeScoreItem(item, idx, "FINISHED", "NBA"),
                ),
            );
          }
        }
      ...
      // Fetch NFL scores
      try {
        const nflRes = await fetch(
          "https://apis.davidcyril.name.ng/sports/nfl/scores",
        );
        if (nflRes.ok) {
          ...
          if (Array.isArray(games) && games.length > 0) {
            pastResults.push(
              ...games
                .slice(0, 10)
                .map((item: any, idx: number) =>
                  normalizeScoreItem(item, idx, "FINISHED", "NFL"),
                ),
            );
          }
        }
      ...
      // Soccer Standings
      try {
        const soccerStandRes = await fetch(
          `https://apis.davidcyril.name.ng/sports/soccer/standings?league=${soccerLeague}`,
        );
        if (soccerStandRes.ok) {
          const soccerData = await soccerStandRes.json();
          const stands = Array.isArray(soccerData)
            ? soccerData
      ...
      // NBA Standings
      try {
        const nbaStandRes = await fetch(
          "https://apis.davidcyril.name.ng/sports/nba/standings",
        );
        if (nbaStandRes.ok) {
          const nbaData = await nbaStandRes.json();
          const stands = Array.isArray(nbaData)
            ? nbaData
            : nbaData.standings || [];
          if (Array.isArray(stands) && stands.length > 0) {
            setStandings((prev: any) => ({
      ...
      // NFL Standings
      try {
        const nflStandRes = await fetch(
          "https://apis.davidcyril.name.ng/sports/nfl/standings",
        );
        if (nflStandRes.ok) {
          const nflData = await nflStandRes.json();
          const stands = Array.isArray(nflData)
            ? nflData
            : nflData.standings || [];
          if (Array.isArray(stands) && stands.length > 0) {
            setStandings((prev: any) => ({
  ...
  const handleLeagueChange = async (leagueId: string) => {
    ...
    try {
      ...
      const soccerRes = await fetch(
        `https://apis.davidcyril.name.ng/sports/soccer/scores?league=${leagueId}`,
      );
      ...
      const soccerStandRes = await fetch(
        `https://apis.davidcyril.name.ng/sports/soccer/standings?league=${leagueId}`,
      );
      if (soccerStandRes.ok) {
        const soccerData = await soccerStandRes.json();
        const stands = Array.isArray(soccerData)
          ...
          <div className="relative z-10 p-6 sm:p-8 md:p-12 flex flex-col justify-between min-h-[250px] sm:min-h-[300px]">
            <div>
              <div className="flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 px-3 py-1 rounded-full text-emerald-400 text-xs font-bold w-fit mb-4 uppercase tracking-widest">
                <Shield size={12} className="text-emerald-400" /> Lenzorah
                Sports Center Live
              </div>
              ...
              <p className="text-xs sm:text-sm text-[var(--rf-text-muted)] max-w-md leading-relaxed">
                Stay updated with absolute real-time live scores, recent match
                history records, and comprehensive standings tables fetched
                directly from active APIs!
              </p>
            </div>
          ...
          <AnimatePresence mode="wait">
            ...
            {activeTab === "live" && (
              ...
              >
                ...
                {/* Soccer League Selector (for Past Scores filtering) */}
                {scoresSubTab === "past" && (
                  <div className="flex flex-wrap sm:flex-nowrap bg-black/20 p-1.5 rounded-xl w-full sm:w-fit gap-1 sm:overflow-x-auto scrollbar-hide border border-white/5 mt-1 mb-4">
                    {SOCCER_LEAGUES.map((lg) => (
                      <button
                        key={lg.id}
                        onClick={() => handleLeagueChange(lg.id)}
                        className={cn(
                          "px-3 py-1.5 rounded-lg text-[9px] sm:text-[10px] font-bold uppercase tracking-wider transition-all whitespace-nowrap",
                          soccerLeague === lg.id
                ...
                {activeScores.length > 0 ? (
                      ...
                      >
                        ...
                        <div className="grid grid-cols-7 items-center justify-center gap-1 sm:gap-2 mb-4 text-center">
                          ...
                          <div className="col-span-3 flex flex-col items-center justify-center gap-1.5 sm:gap-2 min-w-0">
                            ...
                            <h4 className="text-[11px] sm:text-xs md:text-sm font-bold text-white truncate w-full">
                              {score.homeTeam}
                            </h4>
                          </div>
                          ...
                          <div className="col-span-1 flex flex-col items-center justify-center">
                            <div className="flex items-center gap-0.5 sm:gap-1">
                              <span className="text-lg sm:text-xl md:text-3xl font-black text-white">
                                {score.homeScore}
                              </span>
                              <span className="text-[10px] sm:text-xs text-white/30 font-bold">
                                :
                              </span>
                              <span className="text-lg sm:text-xl md:text-3xl font-black text-white">
                                {score.awayScore}
                            ...
                            <span className="text-[7px] sm:text-[8px] text-white/40 uppercase tracking-widest mt-1 block">
                              VS
                            </span>
                          </div>
                        ...
                        {score.eventInfo && (
                          <div className="bg-white/[0.01] border border-white/[0.03] p-2.5 sm:p-3 rounded-2xl flex items-start gap-2 mt-4 text-[9px] sm:text-[10px] md:text-xs text-[var(--rf-text-muted)] leading-relaxed">
                            <span className="text-emerald-400 font-bold uppercase shrink-0 tracking-wider">
                              Info:
                            </span>
                            <p className="flex-1">
                              {score.eventInfo} •{" "}
                  ...
                  <div className="text-center py-16 bg-white/[0.01] border border-white/5 rounded-3xl">
                    <Calendar
                      className="mx-auto text-[var(--rf-text-muted)] mb-3"
                      size={32}
                    />
                    ...
                    <p className="text-xs text-[var(--rf-text-muted)] max-w-xs mx-auto leading-relaxed">
                      ...
                      {scoresSubTab === "live"
                        ? "active live"
                        : "recent completed"}{" "}
                      matches returned by the API scheduler.
                    </p>
                  </div>
                )}
              </motion.div>
            ...
            {activeTab === "standings" && (
              ...
              >
                ...
                {standings[standingsSubTab] &&
                standings[standingsSubTab].length > 0 ? (
                  <>
                    {/* ==========================================
                              ...
                              {standingsSubTab === "soccer" && (
                                <>
                                  <div className="bg-white/[0.01] border border-white/5 rounded-2xl p-2 text-center">
                                    <span className="text-[8px] font-bold text-[var(--rf-text-dim)] uppercase block mb-0.5">
                                      Played
                                    </span>
                                  ...
                                  <div className="bg-white/[0.01] border border-white/5 rounded-2xl p-2 text-center">
                                    <span className="text-[8px] font-bold text-green-400 uppercase block mb-0.5">
                                      W - D - L
                                    </span>
                                    <span className="text-xs font-black text-white">
                      ...
                      <table className="w-full text-left border-collapse">
                        <thead>
                          <tr className="bg-white/[0.03] border-b border-white/5 text-[9px] font-black uppercase tracking-wider text-[var(--rf-text-dim)]">
                            <th className="py-4.5 px-6 w-16 text-center">
                              Rank
                            </th>
                            ...
                            {standingsSubTab === "soccer" && (
                              <>
                                ...
                                <th className="py-4.5 px-6 text-center text-emerald-400">
                                  Points
                                </th>
                              </>
                            )}
                            ...
                            {standingsSubTab === "nfl" && (
                              <>
                                ...
                                <th className="py-4.5 px-4 text-center font-semibold">
                                  PF
                                </th>
                                <th className="py-4.5 px-6 text-center text-emerald-400">
                                  Streak
                        ...
                        <tbody>
                          {standings[standingsSubTab].map(
                            (team: any, index: number) => (
                              ...
                              >
                                <td className="py-4 px-6 text-center font-bold text-white/50">
                                  {team.rank}
                                </td>
                                ...
                                {standingsSubTab === "soccer" && (
                                  <>
                                    ...
                                    <td className="py-4 px-6 text-center font-black text-emerald-400 bg-emerald-500/[0.02]">
                                      {team.points}
                                    </td>
                                  </>
                                )}
                                {standingsSubTab === "nba" && (
                                  <>
                                    <td className="py-4 px-4 text-center text-white/50 font-bold">
                                      {team.conference}
                                    </td>
                                    <td className="py-4 px-4 text-center text-white/70">
                                      {team.wins}
                                ...
                                {standingsSubTab === "nfl" && (
                                  <>
                                    ...
                                    <td className="py-4 px-4 text-center text-white/70 font-semibold">
                                      {team.pf}
                                    </td>
                                    <td className="py-4 px-6 text-center font-black text-emerald-400 bg-emerald-500/[0.02]">
                                      {team.streak}
                  ...
                  <div className="text-center py-16 bg-white/[0.01] border border-white/5 rounded-3xl">
                    ...
                    <p className="text-xs text-[var(--rf-text-muted)] max-w-xs mx-auto leading-relaxed">
                      Standings information is currently not returned by the API
                      servers for this sports league.
                    </p>
                  </div>
                )}
              </motion.div>
